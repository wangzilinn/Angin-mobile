...
class CardOptionsButtonWidget extends StatefulWidget {
  @override
  _CardOptionsButtonWidgetState createState() =>
      _CardOptionsButtonWidgetState();
}
  ...
  Row _buildBottomButton(int state) {
    if (state == 0) {
      return Row(
        ...
        children: <Widget>[
          _buildOneOptionButton(
              Colors.blueAccent, Icons.chat_bubble_outline, "左边按钮填充"),
          _buildOneOptionButton(Colors.blueAccent, Icons.chat, "中间按钮填充"),
          _buildOneOptionButton(Colors.blueAccent, Icons.chat_bubble, "右边按钮填充"),
        ],
      );
    } else {
      return Row(
...
//        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
        children: <Widget>[
          Expanded(child: _buildSeeBackButton()), //去掉Expanded就会有问题
        ],
      );
    }
  }

  Column _buildSeeBackButton() {
    return Column(
      children: <Widget>[
        Container(
  ...
  Column _buildOneOptionButton(Color color, IconData icon, String label) {
    return Column(
